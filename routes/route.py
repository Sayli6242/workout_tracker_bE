...
@router.post("/api/folders/{relative_folder_id}/sections/{section_id}/exercises", response_model=Exercise)
async def create_exercise_in_section(
    relative_folder_id: str, 
    section_id: str, 
    exercise: str = Form(...),  # Will receive JSON string
    ...
    try:
        ...
        image_contents = await uploaded_image.read()
            ...
            # Create exercise document with image data
        exercise_dict = {
            "title": exercise_data["title"],
            "media_url": uploaded_image.filename,  # Store original filename
            "section_id": section_id,
            "folder_id": relative_folder_id,
            "description": exercise_data.get("description", ""),
            "image_data": Binary(image_contents)  # Store actual image data
        }

          # Insert into database
        result = await db.exercises.insert_one(exercise_dict)

         # Prepare response (exclude binary image data from response)
        response_dict = {
...
# Add an endpoint to retrieve the image
